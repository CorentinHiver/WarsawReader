...
namespace Colib
{
  ...
  auto TH1T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return TH1F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return TH1D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return TH1I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto TH2T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return TH2F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return TH2D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return TH2I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto TH3T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return TH3F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return TH3D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return TH3I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto newTH1T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return new TH1F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return new TH1D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return new TH1I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto newTH2T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return new TH2F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return new TH2D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return new TH2I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto newTH3T(ARGS &&... args)
  {
    if constexpr (std::is_same_v<T, float >) return new TH3F(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, double>) return new TH3D(std::forward<ARGS>(args)...);
    if constexpr (std::is_same_v<T, int   >) return new TH3I(std::forward<ARGS>(args)...);
  }

  template <class T, class... ARGS>
  auto vectorToTH1(std::vector<T> const & vector, ARGS &&... args)
  {
    auto ret = newTH1T<T>(std::forward<ARGS>(args)...);
    for (size_t i = 0; i < vector.size(); ++i) ret->SetBinContent(i + 1, vector[i]);
    return ret;
  }

  template <class T, class... ARGS>
  auto vectorToTH1(std::vector<T> const & vector, std::vector<T> const & err, ARGS &&... args)
  {
    auto ret = newTH1T<T>(std::forward<ARGS>(args)...);
    for (size_t i = 0; i < vector.size(); ++i) 
    {
      ret->SetBinContent(i + 1, vector[i]);
      ret->SetBinError(i + 1, err[i]);
    }
    return ret;
  }
