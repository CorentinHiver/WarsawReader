...
namespace Colib
{
  ...
  std::vector<T> append(const std::vector<T>& first, const Args&... rest)
  {
    // 1. Create our return vector as a copy of the first one
    std::vector<T> ret = first;
    
    // 2. Reserve exact memory upfront to avoid expensive reallocations
    ret.reserve(ret.size() + (rest.size() + ... + 0));

    // 3. Copy the elements from all remaining vectors into 'ret'
...
namespace Colib
{
  ...
  {
    // Verifications :
    if (vector.size() == 0) {printc(Color::RED, "In insertionSort(vector, ordered_indexes) : vector size is zero !", Color::RESET); return ordered_indexes;}
  ...
  {
    if (vector.size() == 0) {printc(Color::RED, "In insertionSortPtr(vector, ordered_indexes) : vector size is zero !", Color::RESET); return ordered_indexes;}
    if (vector.size() != ordered_indexes.size()) ordered_indexes.resize(vector.size());
