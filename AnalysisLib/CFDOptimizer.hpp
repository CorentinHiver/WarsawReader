...
public:
  ...
  void calculate_dT(ULong64_t timeRef, ULong64_t time, int label, std::vector<uint16_t> const & trace, double sample_size) 
  {
      ...
      for (int fraction_i = 0; fraction_i<m_parameters.fractions.nb_steps; ++fraction_i) 
    {
      ...
      auto const time_cfd = time + zero*sample_size;
  ...
  double calculateResolution(TH1* histo)
  {
    ...
    return (FWHM_from_FWQM+FWHM) / 2e3;
