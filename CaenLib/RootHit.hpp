...
namespace CaenDataReader
{
  ...
  public:
    // Data fields :
    UInt_t    label         = 0; // Global label (=board_ID*16 + channel_ID*2 + subchannel_ID)
    UShort_t  board_ID      = 0; // Board label [0;max board ID]
    UShort_t  channel_ID    = 0; // Channel label [0;8]
    UShort_t  subchannel_ID = 0; // Sub channel label [0,1]
    Int_t     adc           = 0; // PHA : ADC value. PSD : qshort value.
    Int_t     qlong         = 0; // PHA : unused.    PSD : qlong value.
    ...
    {
      static_assert(true, "Do not use RootHit (RootHit const & other)");
      if (other.trace)
      {
        trace         = new Trace;
        *trace        = *other.trace;
